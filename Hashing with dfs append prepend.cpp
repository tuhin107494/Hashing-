...
ll ans,tot,cs;
 string a;
void dfs(ll u,ll p,ll depth,PLL aval,PLL pval)
{
  ...
  for(auto node:v[u])
  {
     if(node!=p)
     {   
        dfs(node,u,depth+1,append(aval,a[node]),prepend(pval,depth+1,a[node]));
     }
  }
...
/*
    ...
    while(t--)
    {
        ...
        dfs(0,-1,0,append({0LL,0LL},a[0]),prepend({0LL,0LL},0,a[0]));
         ll gcd=__gcd(ans,tot);
         ans/=gcd;
         tot/=gcd;
        cout<<"Case "<<++cs<<": "<<ans<<"/"<<tot<<"\n";
        ans=0,tot=0;
